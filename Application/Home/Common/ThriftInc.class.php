...
if (!isset($GLOBALS['THRIFT_ROOT']))
	$GLOBALS['THRIFT_ROOT'] = APP_PATH."/Home/Common/";
...
function ff_decode_msg($ret, $data)
{
    try{
        $transport = new Thrift\Transport\TMemoryBuffer($data);
        $protocol  = new Thrift\Protocol\TJSONProtocol($transport);
        $tmpIgnore = 0;
        $protocol->readMessageBegin($tmpIgnore, $tmpIgnore, $tmpIgnore);
        $ret->read($protocol);
...
function test()
{
	...
	$req->data = 'OhNice!!!!';
	$ffrpc = new ffrpc_t('112.124.57.159', 1281);
